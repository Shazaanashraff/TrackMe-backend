...
| GET /api/auth/me | integration | tests/integration/auth-me.test.js | returns the signed-in account in the same shape login does, `phoneNumber` included; reflects a manager's edit made after the token was issued; never returns the password; 401 without a token | the account payload changes, or another client needs to re-read itself |
