...
QR pass issue/rotate, driver scanning, and BOARD/ALIGHT attendance events. NOTE: the feature audit flags the token/data model as route-scoped where it should be user-scoped — settle that before extending.
...
`src/routes/qrRoutes.js`, `src/routes/attendanceRoutes.js`, `src/routes/driverBoardingRoutes.js`, `src/controllers/{qr,attendance,boarding,managerAttendance}Controller.js`, `src/utils/qrToken.js`, `src/models/BoardingEvent.js`
