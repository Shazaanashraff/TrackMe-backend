...
// Calculate realistic Sri Lankan vehicle fare based on distance
// Rates: ~0.50-0.80 LKR per km depending on service type
function calculateFare(distance, serviceType) {
  let ratePerKm = 0.65; // Default rate for PUBLIC vehicles
  
  if (serviceType === 'SCHOOL') {
    ratePerKm = 0.55; // School vehicles cheaper
  } else if (serviceType === 'UNIVERSITY') {
    ratePerKm = 0.60; // University vehicles slightly cheaper
...
const SLK_VEHICLE_ROUTES = [
  ...
  },
  {
    ...
    source: 'Colombo Fort',
    destination: 'Negombo Vehicle Stand',
    distance: 37,
    estimatedTime: 90,
    fare: 24,
    serviceType: 'PUBLIC',
    stops: [
      ...
      { stopName: 'Negombo Vehicle Stand', lat: 7.2093, lng: 79.8431, order: 5 }
    ]
  },
  {
    routeId: '103',
    routeName: 'Colombo - Anuradhapura Heritage',
    source: 'Colombo Main Vehicle Stand',
    destination: 'Anuradhapura Vehicle Terminus',
    distance: 205,
    estimatedTime: 330,
    fare: 133,
    serviceType: 'PUBLIC',
    stops: [
      { stopName: 'Colombo Main Vehicle Stand', lat: 6.9365, lng: 79.8578, order: 1 },
      ...
      { stopName: 'Anuradhapura Vehicle Terminus', lat: 8.3369, lng: 80.4133, order: 5 }
    ]
  },
  {
    routeId: '104',
  ...
  },
  {
    ...
    source: 'Galle Fort',
    destination: 'Matara Vehicle Stand',
    distance: 42,
    estimatedTime: 75,
    fare: 27,
    serviceType: 'PUBLIC',
    stops: [
      ...
      { stopName: 'Matara Vehicle Stand', lat: 5.9497, lng: 80.5378, order: 5 }
    ]
  },
  {
    routeId: '106',
  ...
  },
  {
    ...
    source: 'Trincomalee Central Vehicle Stand',
    destination: 'Batticaloa Vehicle Stand',
    distance: 97,
    estimatedTime: 180,
    fare: 63,
    serviceType: 'PUBLIC',
    stops: [
      { stopName: 'Trincomalee Central Vehicle Stand', lat: 8.5711, lng: 81.2344, order: 1 },
      { stopName: 'Kuchchaveli', lat: 8.5889, lng: 81.3167, order: 2 },
      { stopName: 'Nilaveli Beach', lat: 8.6214, lng: 81.2653, order: 3 },
      { stopName: 'Batticaloa Vehicle Stand', lat: 7.7086, lng: 81.6944, order: 4 }
    ]
  },
  {
    routeId: '108',
    ...
    stops: [
      { stopName: 'Colombo Fort', lat: 6.9271, lng: 79.8353, order: 1 },
      { stopName: 'Colombo Harbour', lat: 6.9363, lng: 79.8317, order: 2 },
      { stopName: 'Negombo Vehicle Stand', lat: 7.2093, lng: 79.8431, order: 3 }
    ]
  },
  {
    routeId: '109',
  ...
  },
  {
    ...
    stops: [
      ...
      { stopName: 'Bambalapitiya Vehicle Stop', lat: 6.8833, lng: 79.8600, order: 3 },
  ...
  },
  {
    ...
    destination: 'Puttalam Vehicle Stand',
    distance: 68,
    estimatedTime: 120,
    fare: 44,
    serviceType: 'PUBLIC',
    stops: [
      ...
      { stopName: 'Puttalam Vehicle Stand', lat: 8.0333, lng: 79.8333, order: 4 }
    ]
  },
  {
    routeId: '114',
  ...
  },
  {
    ...
    source: 'Colombo Main Vehicle Stand',
    destination: 'Ratnapura Vehicle Stand',
    distance: 101,
    estimatedTime: 180,
    fare: 66,
    serviceType: 'PUBLIC',
    stops: [
      ...
      { stopName: 'Ratnapura Vehicle Stand', lat: 6.7128, lng: 80.3942, order: 5 }
    ]
  }
];
...
async function seedRoutes() {
  try {
    ...
    const routeIds = SLK_VEHICLE_ROUTES.map(r => r.routeId);
    await Route.deleteMany({ routeId: { $in: routeIds } });
    console.log('🗑️  Cleared existing routes');
    
    const createdRoutes = await Route.insertMany(SLK_VEHICLE_ROUTES);
    console.log(`\n🎉 Successfully seeded ${createdRoutes.length} Sri Lankan vehicle routes into the system\n`);
