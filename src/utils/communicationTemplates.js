...
const PRESETS = [
  ...
  ...[5, 10, 15].map(minutes => ({ id: `traffic_${minutes}`, label: `Traffic · ${minutes} min`, text: `Traffic is causing an estimated ${minutes}-minute delay. I’ll update you if this changes.` })),
...
function canonical(body, role, riderName, broadcast = false) {
  ...
  if (role === 'driver') {
    ...
    if (templateId === 'traffic') return `Traffic is causing an estimated ${minutes(parameters.minutes)}-minute delay. I’ll update you if this changes.`;
